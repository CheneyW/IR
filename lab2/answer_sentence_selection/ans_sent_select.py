    ...
    def extract(self):
        ...
        for qid, data in enumerate(self.preprocessed_data):
            ...
            for sent_idx, sent in enumerate(document):
                features = []
                features += sent_feature[sent_idx]  # 句子本身
                # features.append(len(corpus[sent_idx]) - len(corpus[-1]))
                features.append(unigram_ratio[sent_idx])  # unigram 共现比例
                features.append(bigram_ratio[sent_idx])  # bigram 共现比例
                ...
                features.append(levenshtein_dis[sent_idx])  # 编辑距离
                # features.append(c1)
                # features.append(c2)
        ...
        with open(os.path.join(DIR_PATH, 'svm_rank', 'svmrank_train.dat'), 'w', encoding='utf-8')as f:
            f.write('\n'.join(svm_rank_train))
